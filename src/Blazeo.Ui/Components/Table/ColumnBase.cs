...
/// <summary>
/// Base for a <see cref="DataTable{TItem}"/> child column. Registers itself with the enclosing grid on
...
/// </summary>
/// <typeparam name="TItem">The row data type, cascaded from the <see cref="DataTable{TItem}"/>.</typeparam>
public abstract class ColumnBase<TItem> : ComponentBase, IGridColumn<TItem>, IDisposable
{
    [CascadingParameter] internal DataTable<TItem>? Grid { get; set; }
    ...
    {
        if (Grid is null)
            throw new InvalidOperationException($"{GetType().Name} must be placed inside a <DataTable>.");
    ...
    /// <summary>The display title - overridden by <see cref="PropertyColumn{TItem,TProp}"/> to fall back to the member name.</summary>
