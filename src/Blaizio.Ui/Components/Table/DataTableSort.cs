...
/// <code>DataTableSort&lt;Person&gt;.By(p =&gt; p.LastName).ThenBy(p =&gt; p.FirstName)</code>
/// The chain defines the column's natural ("ascending") order; the header toggle flips the whole chain
/// for the descending pass. Only used for in-memory data - a server-side <see cref="DataTableItemsProvider{TItem}"/>
/// applies its own ordering.
/// </summary>
/// <typeparam name="T">The row data type.</typeparam>
public sealed class DataTableSort<T>
{
    private readonly (Func<T, object?> Key, bool Descending)[] _steps;

    private DataTableSort((Func<T, object?> Key, bool Descending)[] steps) => _steps = steps;

    /// <summary>Start a sort by <paramref name="keySelector"/>, ascending.</summary>
    public static DataTableSort<T> By(Func<T, object?> keySelector) => new([(keySelector, false)]);

    /// <summary>Start a sort by <paramref name="keySelector"/>, descending.</summary>
    public static DataTableSort<T> ByDescending(Func<T, object?> keySelector) => new([(keySelector, true)]);

    /// <summary>Add an ascending tie-breaker, applied only when all earlier keys are equal.</summary>
    public DataTableSort<T> ThenBy(Func<T, object?> keySelector) => new([.._steps, (keySelector, false)]);

    /// <summary>Add a descending tie-breaker, applied only when all earlier keys are equal.</summary>
    public DataTableSort<T> ThenByDescending(Func<T, object?> keySelector) => new([.._steps, (keySelector, true)]);
