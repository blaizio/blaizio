...
/// <summary>
/// The query a <see cref="BzDataTable{TItem}"/> sends its <see cref="DataTableItemsProvider{TItem}"/> for one
...
public sealed record DataTableRequest<TItem>(
    int StartIndex,
    int Count,
    SortState? Sort,
    string? Filter,
...
public sealed record DataTableResult<TItem>(IReadOnlyList<TItem> Items, int TotalCount);

/// <summary>
/// Supplies a <see cref="BzDataTable{TItem}"/> with rows on demand - for server-side or otherwise async
/// data. Given a <see cref="DataTableRequest{TItem}"/> (slice + sort + filter), return that window plus
...
/// </summary>
/// <typeparam name="TItem">The row data type.</typeparam>
public delegate ValueTask<DataTableResult<TItem>> DataTableItemsProvider<TItem>(DataTableRequest<TItem> request);
