...
public class SliderRenderTests : TestContext
{
    ...
    // BzSliderTrack(+Range) followed by one BzSliderThumb per index - the shape the styled layer builds.
    private static RenderFragment Parts(int thumbCount) => builder =>
    {
        var seq = 0;
        builder.OpenComponent<BzSliderTrack>(seq++);
        builder.AddComponentParameter(seq++, nameof(BzSliderTrack.ChildContent), (RenderFragment)(inner =>
        {
            inner.OpenComponent<BzSliderRange>(0);
        ...
        {
            builder.OpenComponent<BzSliderThumb>(seq++);
            builder.AddComponentParameter(seq++, nameof(BzSliderThumb.Index), i);
            builder.CloseComponent();
        }
    };
    ...
    {
        var cut = RenderComponent<BzSlider>(p => p
            .Add(x => x.Min, 0)
            .Add(x => x.Max, 50)
            .Add(x => x.Step, 5)
